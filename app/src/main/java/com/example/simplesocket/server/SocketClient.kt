...
object SocketClient {
     ...
     * 连接服务
     */
    fun connectServer(ipAddress: String, callback: Callback) {
        mCallback = callback
        Thread {
            try {
    ...
    private fun sendHeartbeat() {
        ...
        val msg = "Call Service for Custom"
        clientThreadPool?.execute {
            if (socket == null) {
                mCallback.otherMsg("客户端还未连接")
                return@execute
    ...
    class ClientThread(private val socket: Socket, private val callback: Callback) :
        Thread() {
        override fun run() {
                ...
                while (inputStream.read(buffer).also { len = it } != -1) {
                    ...
                    if (len < 1024) {
                        socket.inetAddress.hostAddress?.let {
                            if (receiveStr == "Copy that for Service") {//收到来自服务端的心跳回复消息
                                Log.i(TAG, "Copy that for Service")
                                //准备回复
                            } else {
